    ...
    if (data && data.Blocks) {
      ...
      Object.values(keyValues).forEach((value, index) => {
        if (
          value.length === 6 &&
          !isNaN(value) &&
          /[.,\/#!$%\^&\*;:{}=\-_`~()]/g.test(value) == false
        ) {
          fileInfo.candidateRefNumbers.push(value);
        }
        if (
          value.match(/\d{2}([\/.-])\d{2}\1\d{4}/g) &&
          Object.keys(keyValues)[index].includes('Invoice')
        ) {
          fileInfo.date = value;
        }
      });
...
module.exports = {
  textractFormResponse: textractFormResponse,
};
